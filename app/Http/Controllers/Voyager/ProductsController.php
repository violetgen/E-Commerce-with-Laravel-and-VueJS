...
class ProductsController extends VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        //we need to know the categories already checked before
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$request->ajax()) {
            $requestNew = $request;
            $requestNew['price'] = $request->price * 100; //just update the price, everyother stuff in the request remain the same
            $this->insertUpdateData($requestNew, $slug, $dataType->editRows, $data);

            event(new BreadDataUpdated($dataType, $data));

            //this is important because is an update, it is supposed to remove the ones there before
    ...
    public function create(Request $request)
    {
        ...
        $allCategories = Category::all();
        $categoriesForProduct = collect([]); //this will be useful when we are editing the added products, so that the categories checked can still be maintained.
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_validate')) {
            ...
            //             'product_id' => $data->id, //getting the product from the request
